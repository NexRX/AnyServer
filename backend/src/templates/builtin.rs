...
const PROMINENCE_2_UUID: Uuid = Uuid::from_bytes([
    0x00, 0xba, 0xfe, 0xed, 0x00, 0x01, 0x40, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04,
]);
...
fn prominence_2_install_steps() -> Vec<PipelineStep> {
    vec![
        step_desc(
            "Download server pack",
            "Download the CurseForge server pack for the selected modpack version.",
            StepAction::DownloadCurseForgeFile {
                file_param: "modpack_version".into(),
                destination: ".".into(),
                filename: Some("server-pack.zip".into()),
                executable: false,
            },
        ),
        step_desc(
            "Extract server pack",
            "Extract the server pack archive into the server directory.",
            StepAction::Extract {
                source: "server-pack.zip".into(),
                destination: Some(".".into()),
                format: ArchiveFormat::Auto,
            },
        ),
        PipelineStep {
            name: "Clean up archive".into(),
            description: Some("Remove the downloaded zip to save disk space.".into()),
            action: StepAction::Delete {
                path: "server-pack.zip".into(),
                recursive: false,
            },
            condition: None,
            continue_on_error: true,
        ...
        step_desc(
            "Write JVM arguments",
            "Configure JVM memory allocation via user_jvm_args.txt (used by the NeoForge/Forge startup script).",
            StepAction::WriteFile {
                path: "user_jvm_args.txt".into(),
                content: "# JVM arguments — managed by AnyServer\n-Xms${memory}\n-Xmx${memory}\n".into(),
            },
        ),
        PipelineStep {
...
fn prominence_2_template() -> ServerTemplate {
    ServerTemplate {
        id: PROMINENCE_2_UUID,
        name: "Prominence II — Hasturian Era".into(),
        description: Some(
            "Prominence II RPG modpack for Minecraft, powered by NeoForge. Downloads the \
             server pack directly from CurseForge — the correct server files are resolved \
             automatically even when the primary listing is a client pack.\n\n\
             **Requirements**:\n\
             - **Java 21+** — use the Java Runtime selector when creating the server.\n\
             - **CurseForge API key** — must be configured by an admin in Admin Panel → CurseForge.\n\
             - **6 GB+ RAM recommended** — modpacks are memory-intensive."
                .into(),
        ),
        config: ServerConfig {
            ...
            parameters: vec![
                ConfigParameter {
                    name: "modpack_version".into(),
                    label: "Modpack Version".into(),
                    description: Some(
                        "Select a Prominence II release to install. The server pack is \
                         downloaded automatically from CurseForge."
                            .into(),
                    ),
                    param_type: ConfigParameterType::CurseForgeFileVersion,
                    default: None,
                    required: true,
                    is_version: true,
                    curseforge_project_id: Some(466901),
                    ..Default::default()
                },
                ConfigParameter {
                    name: "memory".into(),
                    label: "Memory Allocation".into(),
                    description: Some(
                        "JVM heap size. Modpacks need significantly more RAM than vanilla — \
                         6 GB is the minimum, 8 GB+ recommended for larger player counts."
                            .into(),
                    ),
                    param_type: ConfigParameterType::Select,
                    default: Some("6G".into()),
                    required: true,
                    options: vec![
                        "4G".into(),
                        "6G".into(),
                ...
                ConfigParameter {
                    name: "server_port".into(),
                    label: "Server Port".into(),
                    description: Some("The TCP port the Minecraft server listens on.".into()),
                    param_type: ConfigParameterType::Number,
                    default: Some("25565".into()),
                    required: true,
                    ..Default::default()
            ...
            start_steps: vec![],
            install_steps: prominence_2_install_steps(),
            update_steps: {
                // Same as install but skip server.properties (preserve user edits)
                vec![
                    step_desc(
                        "Download server pack",
                        "Download the updated CurseForge server pack.",
                        StepAction::DownloadCurseForgeFile {
                            file_param: "modpack_version".into(),
                            destination: ".".into(),
                            filename: Some("server-pack.zip".into()),
                    ...
                    step_desc(
                        "Extract server pack",
                        "Extract the updated server pack (overwrites mods, configs, etc.).",
                        StepAction::Extract {
                            source: "server-pack.zip".into(),
                            destination: Some(".".into()),
                            format: ArchiveFormat::Auto,
                    ...
                    PipelineStep {
                        name: "Clean up archive".into(),
                        description: Some("Remove the downloaded zip.".into()),
                        action: StepAction::Delete {
                            path: "server-pack.zip".into(),
                            recursive: false,
                        },
                    ...
                    step_desc(
                        "Write JVM arguments",
                        "Re-apply JVM memory settings after update.",
                        StepAction::WriteFile {
                            path: "user_jvm_args.txt".into(),
                            content: "# JVM arguments — managed by AnyServer\n-Xms${memory}\n-Xmx${memory}\n".into(),
                        },
            ...
            isolation: IsolationConfig::default(),
            update_check: Some(UpdateCheck {
                provider: UpdateCheckProvider::CurseForge {
                    project_id: 466901,
                },
                interval_secs: Some(7200),
                cache_secs: 600,
        ...
        for step in steps {
            ...
            match &step.action {
                ...
                StepAction::DownloadCurseForgeFile {
                    ...
                    for v in extract_var_refs(file_param) {
                        refs.push((step_name.clone(), v));
                    }
                    for v in extract_var_refs(destination) {
                        refs.push((step_name.clone(), v));
